...
const hasChildrenEntries = (arr, character) => {
  let flag = false;
  arr.forEach(c => {
    if(Object.entries(c)[0][1] === character){
      if(Object.entries(c)[2][1].length){
        flag = true;
      }
    }
  });
...
const totalCharacters = (arr) => {
  let counter = 0;
  arr.forEach(char => {
    if(Object.values(char)[0]){
      counter += 1;
    }
    if(Object.values(char)[1]){
      counter += 1;
    }
    if(Object.values(char)[2].length){
      counter += Object.values(char)[2].length;
    }
  })
  return counter;
};
